...
def send_emails(**kwargs):
    ...
    users = User.objects.annotate(
        hashtags_num=Count('followed_hashtags')
    ).filter(
        hashtags_num__qt=0
    ).prefetch_related(
        'followed_hashtags'
    ...
    for user in users:
        events = list(
            filter(
                lambda e: True in ['#' + hashtag in e.description for hashtag in user.followed_hashtags],
                recent_events
            )
        ) 
        if events:            
            subject = 'We found few new events for you'
            html_message = render_to_string('emails/new_events_msg.html', { 
                'events': events
            })
